package org.zhang.common.redis.utils;

import lombok.AccessLevel;
import lombok.NoArgsConstructor;
import org.zhang.common.core.utils.SpringUtils;
...
public class QueueUtils {
     ...
     * @param time      延迟时间
     */
    public static <T> void addDzhangedQueueObject(String queueName, T data, long time) {
        addDzhangedQueueObject(queueName, data, time, TimeUnit.MILLISECONDS);
    }
     ...
     * @param timeUnit  单位
     */
    public static <T> void addDzhangedQueueObject(String queueName, T data, long time, TimeUnit timeUnit) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDzhangedQueue<T> dzhangedQueue = CLIENT.getDzhangedQueue(queue);
        dzhangedQueue.offer(data, time, timeUnit);
    }
     ...
     * @param queueName 队列名
     */
    public static <T> T getDzhangedQueueObject(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDzhangedQueue<T> dzhangedQueue = CLIENT.getDzhangedQueue(queue);
        return dzhangedQueue.poll();
    }

    /**
     * 删除延迟队列数据
     */
    public static <T> boolean removeDzhangedQueueObject(String queueName, T data) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDzhangedQueue<T> dzhangedQueue = CLIENT.getDzhangedQueue(queue);
        return dzhangedQueue.remove(data);
    }

    /**
     * 销毁延迟队列 所有阻塞监听 报错
     */
    public static <T> void destroyDzhangedQueue(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDzhangedQueue<T> dzhangedQueue = CLIENT.getDzhangedQueue(queue);
        dzhangedQueue.destroy();
    }
     ...
     */
    public static <T> void subscribeBlockingQueue(String queueName, Function<T, CompletionStage<Void>> consumer, boolean isDzhanged) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        if (isDzhanged) {
            // 订阅延迟队列
            CLIENT.getDzhangedQueue(queue);
        }
